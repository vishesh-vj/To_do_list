...
class Task():
    ...
    def __init__(self, frame):
        ...
        if os.path.exists('userInterface/tasks.txt'):
            ...
            for task in self.tasks:
                ...
                cb = tk.Checkbutton(self.pending_frame, text=task, variable=var, fg='white', bg='#162030',
                                    activebackground="#162030", activeforeground="#C5972A",
                                    selectcolor="#C5972A", command=self.mark_completed)
    ...
    def add_task(self, task_str):
        if task_str.strip():
            ...
            var = tk.IntVar()
            self.vars.append(var)
            cb = tk.Checkbutton(self.pending_frame, text=task_str, variable=var, fg='#D6E4F0',
                                bg='#162030', selectcolor="#C5972A", command=self.mark_completed)
            cb.pack(anchor='w', pady=4)
            self.checkbuttons.append(cb)
...
# ── Clock update ──────────────────────────────────────────────────────────────
def update_time():
    date_label.config(text=strftime('%Y-%m-%d'))   # date first
    time_label.config(text=strftime('%H:%M:%S'))   # time second
...
# ── Header: title (left) + clock (right) in one row ──────────────────────────
...
# Date — gold matches the accent colour used throughout the UI
date_label = tk.Label(
    clock_frame,
...
# Time — soft blue-white matches the task text colour
time_label = tk.Label(
    clock_frame,
...
add_tsk_btn = tk.Button(input_card, text='+ ADD TASK', font=("Trebuchet MS", 10, "bold"),
                        fg="black", bg="#C5972A", activebackground="#C5972A",
                        width=12, height=1, command=lambda: ob.add_task(entry.get()))
...
del_tsk_btn = tk.Button(root, text="Clear Completed Tasks", width=25,
                        command=ob.clear_table, fg='#8BAFC8', bg='#1E3A52',
